...
namespace {
    ...
    const DWORD kHardGapMaxEndMs  = 3500;
    const DWORD kHardChildLifeMin = 2500;
    ...
    const int   kCursorPopWRange = 90;
    const int   kCursorPopH      = 110;
    const int   kCursorPopHRange = 70;

    // 下面这些是**运行时**值：BeginRansom 里按 settings::Hardcore() 定下来，
    // 整轮勒索里不再变（演出中途不允许切换模式）。
    bool  g_hardcore      = false;
    int   g_maxChildren   = kMaxChildren;
    DWORD g_gapMin        = kGapMinMs;
    DWORD g_gapMax        = kGapMaxMs;
    DWORD g_gapMinEnd     = kGapMinEndMs;
    ...
    // 倒计时快归零时的间隔与批量见文件上方的 kGapMinEndMs / kGapMaxEndMs
    // —— 它们得排在运行时变量前面（那里要用初值），所以提前了。
    ...
    DWORD NextWaveGap()
    {
        ...
        const double lo = (double)g_gapMin + ((double)g_gapMinEnd - (double)g_gapMin) * p;
        const double hi = (double)g_gapMax + ((double)g_gapMaxEnd - (double)g_gapMax) * p;
    ...
    // atCursor=true：开在**当前鼠标位置**，尺寸取小一号。
    // 硬核用它做"阻碍点击" —— 玩家要点的东西会先点到这个窗口上。
    // 挡住点击这件事只能靠真实窗口：aero 的窗口是分层窗口，
    // 光标下的非透明像素会收到鼠标消息，所以开在光标下面就是真的挡。
    void SpawnChild(DWORD lifeMs, bool atCursor = false)
    {
        if ((int)g_children.size() >= g_maxChildren) return;
        ...
        const int w = atCursor ? (kCursorPopW + rand() % kCursorPopWRange)
                               : (170 + rand() % 250);   // 170 - 419
        const int h = atCursor ? (kCursorPopH + rand() % kCursorPopHRange)
                               : (140 + rand() % 190);   // 140 - 329
    ...
    void Tick()
    {
        ...
        if (g_burstDone && now >= g_nextWave)
        {
            ...
            for (int i = 0; i < n; ++i)
            {
                const DWORD life = g_childLifeMin + (DWORD)(rand() % (g_childLifeMax - g_childLifeMin));
                SpawnChild(life);
            }
            elog::Write(L"[popup] 零星冒出 %d 个子窗口（存活 %lu-%lums）",
                n, (unsigned long)g_childLifeMin, (unsigned long)g_childLifeMax);

            g_nextWave = now + NextWaveGap();
        }

        // ---- 硬核：鼠标位"阻碍弹窗" ----
        // 单独一条时间线，不挤占上面那批的名额判断：上限到了就直接跳过，
        // 等有窗口关掉之后自然又会冒。
...
namespace popup {
    ...
    void BeginRansom(int childBurst)
    {
        ...
        if (childBurst > g_maxChildren) childBurst = g_maxChildren;

        g_total = 0;
        g_children.reserve(g_maxChildren);
