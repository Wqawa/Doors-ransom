...
// 搜索半径必须盖住 |Ha - Hs|，其中 Ha = kSHop / factor 是分析步长。
//
// 这里踩过一次：原来是 80，刚好够"慢放"用 ——
//   factor 1.125（80 秒 -> 90 秒）：Ha = 455，|Ha-Hs| = 57 < 80  ✓
// 但硬核主题曲要"快进"（210 秒 -> 180 秒，factor 0.8571）：
//   Ha = 597.3，|Ha-Hs| = 85.3 > 80  ✗
// 搜不到上一帧的自然延续点，WSOLA 只能挑到搜索窗口的边界候选，
// 听感上就是接缝/抖动。所以提到 160，两种方向都留了余量。
// 代价是候选数 161 -> 321（搜索开销约 2 倍），一次性加工可以接受。
const int kSSrch = 160;
