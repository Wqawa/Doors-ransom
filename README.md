...
- **假金币。** 生成的金币里会混进约 35% 的"假币"，它们只是文件名被同形字替换过
  （`Gold_50` → `G01d_5o`），双击**一分钱都不进账**，而是按被污染的位置吃惩罚：
  前缀 `Gold` 被换掉 → 倒计时扣「面额 × 0.1」秒；数字被换掉 → 未付的赎金 +面额；
  两段都换掉 → 两种同时触发。被顶上去的赎金**不是永久的** ——
  之后捡到的真金币会先拿去还这笔债，还完读数就回到正常节奏。
